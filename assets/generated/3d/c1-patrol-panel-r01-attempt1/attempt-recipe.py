...
for mat in (iron, bronze, dark, paper, ceramic, amber):
    members = [o for o in meshes if o.data.materials[0] == mat]
    if not members:
        continue
    bpy.ops.object.select_all(action="DESELECT")
    for obj in members:
...
print(json.dumps({"output": str(OUT), "triangles": measurements["triangles"], "meshes": len(meshes), "texture_bytes": measurements["texture_bytes"], "runtimeEligible": False}))
