...
def hand(surface, side, transform=None, bone_prefix=''):
    ...
    for index,(name,x,length) in enumerate(digits):
        ...
        for j in (1,2):
            p = joints[j]
            surface.tube([(p[0]-r*.55,p[1]+.001,.005),(p[0]+r*.55,p[1]+.001,.005)],
                         [.0006]*2,'SkinShade',{names[j-1]:.5,names[j]:.5},6)
    ...
    bones.append((B('Contact'),(0,-.07,-.035),(0,-.095,-.035),B('Wrist')))
    if transform:
...
def stage_model():
    if scene().get('modeled'):
        raise RuntimeError('Model already exists; preserve candidate before explicit new revision')
...
def pose_hand(arm,clip,t):
    ...
    for j in (1,2,3):
        ...
        p.rotation_euler.x=-curl*.38
        p.rotation_euler.y=side*curl*.26 if j==1 else 0
        p.rotation_euler.z=side*curl*.26
...
def glb_info(path):
    ...
    for a in doc.get('animations',[]):
        durations=[doc['accessors'][s['input']]['max'][0]-doc['accessors'][s['input']]['min'][0]
                   for s in a['samplers']]
        clips.append({'name':a.get('name'),'durationSeconds':max(durations),'channels':len(a['channels'])})
...
def fbx_info(path):
    ...
    def visit(elem):
        if elem.id==b'AnimationStack':
            ...
            stacks.append({'name':name,'durationSeconds':
                (values.get('LocalStop',0)-values.get('LocalStart',0))/46186158000})
        for child in elem.elems:
            visit(child)
    visit(root)
...
def manifest():
    ...
    return {'rfc':'RFC-CX-018','runtimeEligible':False,'promotedBy':None,
        ...
        'rigs':[{'asset':o['asset'],'armature':o.name,'boneCount':len(o.data.bones),
            ...
            'clips':[{'name':c['clip'],'takeName':c['clip'],'firstFrame':0,
                      'lastFrame':c['endFrame']-c['startFrame'],
                      'durationSeconds':c['durationSeconds'],'contactMs':c['contactMs']}
                     for c in json.loads(s.get('clips','[]')) if c['asset']==o['asset']],
        ...
        'handFrames':{'wristOriginBlender':[0,0,0],'fingersBlender':[0,-1,0],
            ...
            'contactBone':'Contact','contactCenterRestBlender':[0,-.07,-.035],
