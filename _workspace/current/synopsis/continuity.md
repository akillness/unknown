...
| 불변식 | 검사 | 결과 |
|---|---|---|
| P1 이중 경로 | K1~K10 각 필수 명제에 루트가 서로 다른 대체 확정 경로 2쌍 이상 | **unknown / blocked (M23 R4)**. §5.1은 비트별 최소쌍·재확인 후보의 전사이지, 동일 명제를 각각 완결하는 대체 경로·해금 조건·도달성을 증명한 표가 아니다. K1의 `t0-b3`/`c2-b3`, K2의 `c1-b2`/`c4-b2`는 각각 같은 루트쌍의 재확인이므로 두 독립 대체쌍으로 세지 않는다. K8의 재확인 누락도 남는다. 후보쌍이 둘이어도 의미·도달성 저작 증거 없이는 PASS가 아니다 |
| P2 불파괴 | P1의 두 경로 중 한 쌍의 **양쪽 매체**가 모든 플레이어 행동으로부터 보호되고 서로의 사본이 아님 | **unknown / blocked (M23 R4)**. 첫 판독 자동 보존은 읽은 뒤의 보존 계약이며, 두 매체 모두의 첫 열람 전 접근과 모든 파괴·선택 행동 후 보존을 전수 증명하지 않는다. 한쪽 보호 또는 “위와 같음” 같은 메모로 완전한 불파괴 쌍을 승인하지 않는다 |
...
**역사적 수치의 범위 정정 (2026-09-13, M23 R4):** 이전 R7의 P1 “기계 검증 9/10 · 미검증 1”은 최소쌍이 있는 확정/재확인 비트의 후보 집계였으며 P1 강한 보존 증명이 아니었다. P2 “10/10 충족 [INFERENCE]”도 문서상 첫 판독 보존 추론이었다. 두 수치를 현재 P1/P2 통과 증거로 **철회**하며, K8 재확인 비트 하나만 추가해도 자동 회복되지 않는다. §5.0의 과거 C-07 실행·계보 기록과 §5.1의 K 인용 자체는 보존한다. 별도 `planning/audit-campaign-preservation.mjs`가 live 계보·명시적 쌍·중복 루트와 미충족 증거를 분리하고, 후보를 저작된 대체 경로나 불파괴 사실로 바꾸지 않는다. 본 변경은 전체 G1 PASS가 아니다.
