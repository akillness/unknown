...
## RFC-CX-015 — 네이티브 실동작 플레이테스트·모니터링과 게임플레이 영상 (2026-09-11)

[OBSERVED] id 충돌 정정: 본 블록은 최초 `RFC-CX-013`으로 적었으나, 같은 시각 병행 세션 2개가 각각 `RFC-CX-013`(M7 컨셉 리소스 런타임 반영 / RFC-CX-012 이월 해소)을 선점했다. 3중 충돌을 피해 본 작업을 **RFC-CX-015**로 재번호한다(014는 예비). 병행 세션의 블록은 건드리지 않았다. CLAUDE.md §10.1 병행 세션 안전 규칙의 실사례로 기록한다.
...
### RFC-CX-013 delivery evidence — M7 컨셉 리소스 T0 스테이지1 런타임 배선 (2026-09-11)

- [OBSERVED] 사용자 지시(2026-09-11): "이어서 디벨롭까지 진행해보자. 코어룰프 스테이지1, 리소스가 전혀업데이트되지않았네 ui/ux, 연출, gui, 모두야". 위 RFC-CX-013 계획 블록의 3레인 병렬(HubShell/ReaderStage/UiSkin)을 집행하고 디렉터가 종료 시 1회 배치 검증했다. Unity 6000.5.6f1 · URP 17.5.0 · `unity/Unknown`. 모든 배치 실행 인자 `-batchmode -nographics --burst-disable-compilation`(캡처만 그래픽 사용).
- [OBSERVED] **HubShell**(modeling↔systems): 후보 `Assets/_Project/Art/Candidates/m7-hub-r01/` · 빌더 `Editor/M7HubProjectBuilder.cs`(`Tools/M7/Import hub shell candidates`, `Tools/M7/Approve hub shell (director only)`) · 프로파일 `Presentation/M7HubProfile.cs` → `Assets/_Project/Resources/M7Hub.asset` · 런타임 부분클래스 `App/M7HubSession.cs`(`BindM7Hub`, 게이트 `runtimeApproved || --m7-hub-diagnostic`). 임포트 로그 `M7_HUB_SHELL_IMPORTED textures=6 materials=3 runtimeApproved=false`.
- [OBSERVED] **ReaderStage**(presentation↔systems): 후보 `Art/Candidates/m7-reader-r01/` · 빌더 `Editor/M7ReaderProjectBuilder.cs`(`Tools/M7/Import reader stage candidates`, `Tools/M7/Approve reader stage (director only)`) · 프로파일 `Presentation/M7ReaderStageProfile.cs` → `Resources/M7ReaderStage.asset` · 런타임 `App/M7ReaderSession.cs`(`M7ReaderStageEnabled`, 게이트 `runtimeApproved || --m7-reader-diagnostic`). 임포트 `M7_READER_STAGE_IMPORTED fbx=2 textures=8 materials=7 runtimeApproved=false`. 카메라는 임포트된 인스턴스 bounds에서 산출 — `lookAt [0.0914999843, 0.195000052, -2.98023224e-08]` · `cameraPosition [-0.458500028, 0.615, 1.15]` · `horizontalFov 48`(`Builds/m7-reader-import-audit.json`).
- [OBSERVED] **UiSkin**(presentation↔systems): 후보 `Art/Candidates/m7-ui-r01/` · 빌더 `Editor/M7UiSkinProjectBuilder.cs`(`Tools/M7/Import UI skin candidates`, `Tools/M7/Approve UI skin (director only)`) · 프로파일 `Presentation/M7UiSkinProfile.cs` → `Resources/M7UiSkin.asset` · 런타임 `App/M7UiSession.cs`(`ApplyM7UiSkin(GameScreen)`, 게이트 `runtimeApproved || --m7-ui-diagnostic`). 임포트 `M7_UI_SKIN_IMPORTED textures=2 runtimeApproved=false`. 3레인 일괄 진입점 = `Tools/M7/Import all M7 candidates`(`Tide.EditorTools.M7ProjectBuilder.ImportAll`, exit 0, `M7_IMPORT_ALL_DONE`).
- [OBSERVED] 임포트 감사 3종: `unity/Unknown/Builds/m7-{hub,ui,reader}-import-audit.json`. 로그 `systems/tech-verification/concept-first-m7/runtime/m7-import.log`.
- [OBSERVED] 자동화 검증(exit 0): **EditMode 53/53**(`-assemblyNames Tide.Tests.Sim`, total 53 · passed 53 · failed 0 · skipped 0, `runtime/editmode.xml`), **PlayMode 78/78**(10개 클래스 `-testFilter`, total 78 · passed 78 · failed 0 · skipped 0 · inconclusive 0, `runtime/playmode.xml`). 클래스별: T0PlayModeTests 15 · M5DirectionPlayModeTests 15 · ReviewNotesPlayModeTests 15 · C1SignaturePlayModeTests 10 · C1PlayModeTests 7 · T0ResourceInteractionTests 6 · M7UiSkinPlayModeTests 3 · M7ReaderPlayModeTests 3 · M7HubPlayModeTests 2 · T0CaseThreadTests 2.
- [OBSERVED] 신규 M7 테스트 8건 전부 Passed: `M7HubPlayModeTests.GateOffLeavesCommittedHubShellUntouched` · `M7HubPlayModeTests.GateOnSwapsListedRootsAddsOneLampAndFlatAmbientOnce` · `M7ReaderPlayModeTests.GateOffNeverEntersReaderStage` · `M7ReaderPlayModeTests.GateOnBuildsStageWithTwoLightsAndTearsDownOnExit` · `M7ReaderPlayModeTests.GateOnReducedMotionKeepsCrankAtRest` · `M7UiSkinPlayModeTests.GateOffKeepsCommittedLiteralsAndNoSkinBackings` · `M7UiSkinPlayModeTests.GateOnInsertsNonRaycastBackingsAndKeepsButtonsClickable` · `M7UiSkinPlayModeTests.GateOnAtTextScale150RendersWithoutLayoutChange`. **skipped=0** 이므로 `Assert.Ignore` 폴백이 한 건도 발동하지 않았다 — 후보 에셋이 실재했고 게이트 off/on 두 경로가 모두 실제로 실행됐다.
- [OBSERVED] 네이티브 캡처(`Tide.EditorTools.M7ProjectBuilder.Capture`, 그래픽 사용, exit 0, graphicsDevice Metal): 1280x800 PNG **7장** → `unity/Unknown/Builds/m7-diagnostics/` + `m7-diagnostics.json`. 샷: `hub-before.png` · `hub-after-m7-shell.png` · `hub-after-m7-shell-and-ui-skin.png` · `hub-after-m7-viewport.png` · `reader-stage-rest.png` · `reader-stage-viewport.png` · `reader-stage-crank-150deg.png`. 리포트 수치: `crankPivotFound true` · `crankStrokeDeg 150.0` · `readerStageBounds.size [1.48191333, 0.6113224, 0.8704778]` m · `readerTriangles 8028` · `assets.runtimeApproved {hub:false, ui:false, reader:false}`. 캡처는 후보를 **메모리에서만** 적용하며 허브 씬을 저장하지 않는다.
- [OBSERVED] FBX 후보 계보(`scripts/blender/export_m7_fbx.py`, Blender 5.1.2 headless, `assets/generated/3d/concept-first-m7/fbx/fbx-export-report.json`): 4파일 approx tris — SM_Env_Watchroom 30818 · SM_Prop_OpticalReader 7824 · SM_Prop_RecordSet 204 · SM_Env_GateThree 34928. 원본 blend sha는 내보내기 전후 동일, 텍스처 비임베드.
- [OBSERVED] 이번 집행에서 확인된 발견 5건:
  1. `-batchmode`를 **그래픽과 함께** 실행하면 Burst 백그라운드 컴파일러에서 segv(`Burst.Compiler.IL…` 네이티브 스택, exit 139). 이후 M7 배치 실행은 전부 `--burst-disable-compilation`을 붙인다. 프로젝트 코드 원인이 아니다.
  2. `EditorSceneManager.OpenScene`/`NewScene`은 매니지드 참조로만 도달 가능한 에셋을 언로드한다 — 로드해 둔 프로파일의 `Material`/`GameObject` 필드가 null이 되어 첫 캡처가 5장이 아니라 1장만 냈다. 수리: 씬 전환마다 세 프로파일을 `Resources.Load`로 **재로드**(`Editor/M7ProjectBuilder.cs` `Reload()`).
  3. FBX 축 변환이 X를 미러링한다. 그래서 판독기 스테이지 카메라 측면은 Blender 공간 가정이 아니라 **임포트된 인스턴스에서 실측**한다(크랭크 측 = pivot의 world x, 정면 = `rd-thumbscrew` z).
  4. `hub-after-m7-viewport.png`에서 타공강 작업대가 현재 타일링/조명에서 거의 평평한 어두운 덩어리로 읽힌다 — 승격 전 타일링·밝기 튜닝 필요. 염-콘크리트 벽/바닥과 조위 띠는 정상으로 읽힌다.
  5. 청동 프레임 배킹 틴트를 `(.5,.5,.5,.72)`로 낮춰 헤더/툴바 텍스트가 텍스처 위에서 대비를 유지하게 했다.
