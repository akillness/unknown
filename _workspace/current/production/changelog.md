...
- 이전 연습 빌드6c88870f에 해당 빌드의 소스20개를 연결하고5d7b18c Git blob의 바이트/SHA로 확인했다. 현행 생산 소스7e2c383·앱ba14f79e와 구분한다. 완료 시각도 최신 검증 이후로 갱신했다. 런타임·빌드·생산 gate는 바꾸지 않는다.

## 2026-09-14 · M24 캐릭터 마감과 실제 플레이 영상

- 한서린의 얼굴 윤곽·눈/코/입, 비대칭 머리 면/가닥, 소매 주름과 앞치마 봉제선/리벳을 Blender에서 개정했다. 전신19,618 tri/50본으로 기존20k 상한을 유지했다. 양손 형상·본·11 take·접촉 시간은 바꾸지 않았다.
...
## 2026-09-18 · M27 실마리 명료화 — 체크리스트·단계 표기·데이터 기반 안내·힌트 설정·글리프 (RFC-CX-M27-20260918)

- **클루 명확**: 사건 흐름 카드 아래 **요구조건 칩**(✓/▶/○ + 글리프 + 짧은 캡션), `다음:` = 첫 미충족 조건의 라벨. t0-b3에서 인용 2/2인데 결손 끝점이 틀리면 「결손 구간」 칩이 열린 채 남는다.
- **확장 가능**: 라벨·캡션·단계 순서는 `beats.json requires[]`(emit-tables 술어 맵)에서 오고 검증기 T0-06이 지킨다. 런타임에 비트 리터럴 분기를 추가하지 않았다.
- **중간 지점**: 카드 `단계 i/n · 제목` + `조건 d/t`, 부제·교습 헤더·안내 제목이 비트 id 대신 단계 표기. 저장 복원 상태문이 단계·조건 수를 말한다.
- **진행 보장·가이드**: 안내 오버레이 「이 단계의 조건」 전체 라벨 목록; C1 절은 순찰/서명 패킷의 목표·관찰·조건·확정에서 도출; C1 카드 문구 문자열표 이동. 결손 구간 끝점 규칙(시작 = 첫 결손 눈금, 끝 = 기록이 돌아온 첫 눈금)을 절차 힌트와 라벨에 명시.
- **밸런스**: t0-b1·t0-b2 목표문 행동문으로 교체, 힌트 제안 토스트 on/off(기본 on). 수치 밴드 무변경(n=0) — `balance/m27-play-balance.md`.
- **리소스 UI**: Higgsfield 체크리스트 글리프 3종(로컬 개발 프로필), 힌트 토스트 테두리 칩, 질문 카드 배경 uv 중앙 크롭, 죽은 접근자 삭제.
- **검증**: QA 루프 GREEN `20260919T164740Z` — EditMode 65/65 · PlayMode 148(147/0/1) · boot 1/1 · M27 6/6. 빌드 445,068,423 B. 루프가 잡은 회귀 3건은 같은 세션에 해소.
- **배포**: `main` push + GitHub Release `v0.27.0-dev`(prerelease, 미서명 개발 빌드 zip).
