...
```bash
bash .Codex/skills/game-ops-harness/scripts/bootstrap-workspace.sh "$(pwd)"
```
...
```bash
bash .Codex/skills/game-ops-harness/scripts/session-start.sh "<one-line task>"
```
...
- Codex with `CLAUDE_CODE_EXPERIMENTAL_AGENT_TEAMS=1`: `TeamCreate` with the
  required lanes from `.Codex/agents/game-*.md`; use `TaskCreate`/`TaskUpdate` for the
...
→ `zg index --rebuild` if the workspace layout changed → re-derive root `AGENTS.md` if a
...
6. Re-derive `AGENTS.md` when the harness itself changes — the rule file is a cycle artifact.
