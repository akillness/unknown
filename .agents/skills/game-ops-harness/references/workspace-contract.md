...
## Frontmatter (every Markdown artifact under current/ and archive/)
...
```
The script changes Markdown status before archival, then moves it without editing immutable history afterward. Default tracked paths use git mv and stage those paths; untracked files do not stage. --no-stage preserves the entire index for both. --root permits any cwd. Missing sources, escapes, symlink components and archive overwrite are rejected. Output is the successor supersedes path.
...
JSON/CSV/HTML/source/binaries do not receive Markdown frontmatter. Use same-basename .meta.md or a bundle README manifest with owner/cycle/source/hash. Freshness validates Markdown structure, not program results or memory_sync. product/ belongs to game-product-manager; economy/ remains in-game fairness/resources.
