...
namespace Tide.Input {
 public sealed class WatchInput:MonoBehaviour {
  ...
  public void Initialize(string json,string overrides=null){
   ...
   map.FindAction("Overlay").performed+=c=>{if(c.ReadValue<float>()<=0)return;var key=Role(c);bool lb=Gamepad.current?.leftShoulder.isPressed??false;Overlay?.Invoke(key=="evidence"?"evidence":key=="hypothesis"?"hypothesis":key=="hints"?"hints":key=="guide"?"guide":key=="receipt"?"receipt":lb?"hypothesis":"evidence");};
