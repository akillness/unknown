...
namespace Tide.Tests
{
    public sealed class C1SignaturePlayModeTests
    {
        ...
        [UnityTest] public IEnumerator DefaultGuidanceDoesNotRevealHumidityOrInventPageValue()
        {
            ...
            Assert.IsTrue(game.SignatureActive);Assert.IsFalse(game.SignatureComplete);Assert.AreEqual(1,host.GetComponentsInChildren<ScrollRect>().Single(s=>s.viewport.name=="Viewport").verticalNormalizedPosition);var text=string.Join("\n",Texts().Select(t=>t.text));StringAssert.DoesNotContain("가장자리가 풀리고",text);StringAssert.DoesNotContain("1단이",text);StringAssert.DoesNotContain("같은 출처",text);Assert.IsFalse(Texts().Any(t=>t.name=="Single page number"));Assert.IsTrue(HasMask());
            Click("c1-signature-open-"+game.Definition.Signature.LeftClue);yield return null;Assert.AreEqual(1,host.GetComponentsInChildren<ScrollRect>().Single(s=>s.viewport.name=="Viewport").verticalNormalizedPosition);Do("ObserveSignature",game.Definition.Signature.LeftClue);yield return null;Assert.AreEqual("번호 표기",Texts().Single(t=>t.name=="Single page number").text);
        }
