...
yield return null;if(Directory.Exists(directory))Directory.Delete(directory,true);}
  ...
  [UnityTest] public IEnumerator RecoveryNewSlotIsSelectableAfterSessionRestart(){var future=JournalSave.Encode(game.Journal,"future",20000,6291456);future["schemaVersion"]=3;yield return Wait(game.Store.WriteAsync(future));var original=File.ReadAllBytes(Path.Combine(directory,"save.json"));yield return Restart();Assert.AreEqual("recovery",game.Surface);Click("recovery-new-slot");Click("start");Click("handover");Click("line-hb-l1");yield return Wait(game.FlushSaves());Assert.AreNotEqual(directory,game.Store.DirectoryPath);yield return Restart();Assert.AreEqual("recovery",game.Surface);Click("select-slot-1");Click("start");Assert.AreNotEqual("recovery",game.Surface);Assert.IsTrue(game.Journal.State.Has("line:rec-handover-brief:hb-l1"));CollectionAssert.AreEqual(original,File.ReadAllBytes(Path.Combine(directory,"save.json")));}
  [UnityTest] public IEnumerator ValidatedRetryClearsRefusalAndPreservesSaveIdentity(){var valid=JournalSave.Encode(game.Journal,"valid",20000,6291456);yield return Wait(game.Store.WriteAsync(valid));var future=(Newtonsoft.Json.Linq.JObject)valid.DeepClone();future["schemaVersion"]=3;future["commitIdempotencyKey"]="future";yield return Wait(game.Store.WriteAsync(future));yield return Restart();Assert.AreEqual("recovery",game.Surface);File.Copy(Path.Combine(directory,"save.bak"),Path.Combine(directory,"save.json"),true);Click("recovery-retry");Click("start");Assert.AreNotEqual("recovery",game.Surface);var commit=game.CommitAsync(new PuzzleCommand("ViewLine","rec-handover-brief","hb-l1"));yield return Wait(commit);Assert.IsTrue(commit.Result);var loaded=game.Store.Load();Assert.AreEqual((string)valid["saveId"],(string)loaded.Document["saveId"]);Assert.AreEqual((string)valid["createdUtc"],(string)loaded.Document["createdUtc"]);}
