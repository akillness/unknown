...
[OBSERVED] M12 native playtest (RFC-CX-015, id renumbered from a 3-way collision with parallel sessions): the shipped macOS player was driven in a real window (cliclick pointer; synthetic key events never reach the Unity Input System, so keyboard/controller equivalence stays with PlayMode). Full T0 b1→b3 with preview→confirm, undo/redo (headSeq 31→30→31), OS restart resume, C1 patrol confirm, and C1 review notes (source-original correctly withheld). Player.log exceptions 0; save.json watched live.
[OBSERVED] Three defects only real operation surfaced, fixed and re-verified live: D-M9-15 question panel rendered above its button and was pushed off-screen by the focus scroll (now anchored beneath the action via `ReviewNotesView.QuestionAnchorId`); D-M9-16 resume status always showed the t0-b1 welcome (`ResumeStatus()`); D-M9-17 internal ids leaked into player strings at 4 sites (`ReviewMediaName(sourceType)`). Post-fix EditMode 53/53, PlayMode 73/73 (+6 skipped, parallel M7 tests awaiting their own assets), boot 1/1, build 354247240 B.
[OBSERVED] Unplanned but useful: closing the window mid-recording killed the player; relaunching on the same save restored 1/2 citations with save.json sha unchanged — real evidence for the atomic-save ladder.
[CARRIED] Wheel scroll does not reach long action lists (drag works) — a genuine UX gap, deferred. Clue display names, t0-b2 authoring-note leak, M7 prefab rebuild, human playtests all remain open. `screencapture` needs `nohup`+`disown` to survive a bash call.
